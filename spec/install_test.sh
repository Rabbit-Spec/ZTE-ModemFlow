...
curl -sSL --connect-timeout 10 --retry 3 -o /config/shell/zte_monitor.sh "${RAW_URL}/scripts/zte_monitor_test.sh" || {
...
curl -sSL --connect-timeout 10 --retry 3 -o /config/packages/zte_modemflow.yaml "${RAW_URL}/packages/zte_modemflow_test.yaml" || {
...
echo -e "     └─ ${BLUE}https://raw.githubusercontent.com/Rabbit-Spec/ZTE-ModemFlow/main/dashboards/dashboard_test.yaml${NC}\n"
